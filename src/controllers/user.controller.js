...
const getUserChannelProfile = asyncHandler(async(req, res) => {
    ...
    const channel = await User.aggregate([
        ...
        },
        {
            $addFields: {
                ...
                isSubscribed: {//for subscription button
                    $cond: {
                        if: {$in: [req.user?._id, "$subscribers.subscriber"]},//if is it present in subscriber or not (in check inside the object)
                        then: true,
                        else: false
                    }
                }
        ...
        {
            $project: {//sending the particular fields for projection
                fullName: 1,
                username: 1,
                subscribersCount: 1,
    ...
    .status(200)
    .json(
        new ApiResponse(200, channel[0], "User channel fetched successfully")//channel[0] gives the user
    )
})
